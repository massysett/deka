{-# LANGUAGE Safe #-}
module Deka.Internal.Double.Decoding where
...
instance Enum Coefficient where
  toEnum i
    | i < 0 = error $ "Deka.Double.Coefficient.toEnum: argument "
      ++ "out of range; is negative"
    | length r > coefficientLen = error $ "Deka.Double.Coefficient."
  ...
  fromEnum i
    | r > (fromIntegral (maxBound :: Int)) =
        error $ "Deka.Double.Coefficient.fromEnum:"
...
instance Enum Payload where
  toEnum i
    | i < 0 = error $ "Deka.Double.Payload.toEnum: argument "
      ++ "out of range; is negative"
    | length r > payloadLen = error $ "Deka.Double.Payload."
        ++ "toEnum: argument too large"
    | otherwise = Payload r
    where
      r = integralToDigits i
  ...
  fromEnum i
    | r > (fromIntegral (maxBound :: Int)) =
        error $ "Deka.Double.Payload.fromEnum:"
          ++ " argument too large to fit into Int"
    | otherwise = fromIntegral r
    where
...
coefficientLen :: Int
coefficientLen = c'DECDOUBLE_Pmax

-- | Maximum number of digits in a payload.
payloadLen :: Int
payloadLen = c'DECDOUBLE_Pmax - 1
...
--
-- Also, the upper bound is different too, becuase decDouble is
...
minMaxExp = (l, h)
  where
    l = c'DECDOUBLE_Emin - c'DECDOUBLE_Pmax + 1
    h = c'DECDOUBLE_Emax - c'DECDOUBLE_Pmax + 1
...
minNormalAdj = AdjustedExp c'DECDOUBLE_Emin
...
toDecNumberBCD (Decoded s v) = (e, ds, sgn)
  where
    ...
    (e, ds) = case v of
      Infinite -> (c'DECFLOAT_Inf, replicate c'DECDOUBLE_Pmax 0)
      NaN n (Payload ps) -> (ns, np)
        where
          ...
          pad = replicate (c'DECDOUBLE_Pmax - length ps) 0
      Finite (Coefficient digs) (Exponent ex) ->
        ( fromIntegral ex, pad ++ map digitToInt digs )
        where
          pad = replicate (c'DECDOUBLE_Pmax - length digs) 0

-- | Encodes a new 'Double'.
fromBCD :: Decoded -> IO Double
fromBCD dcd =
  newDouble >>= \d ->
  withForeignPtr (unDouble d) $ \pD ->
  let (expn, digs, sgn) = toDecNumberBCD dcd in
  withArray digs $ \pArr ->
  c'decDoubleFromBCD pD expn pArr sgn >>
  return d

-- | A Double with coefficient 'D1', exponent 0, and sign 'NonNeg'.
one :: IO Double
one = fromBCD
  $ Decoded NonNeg (Finite (Coefficient [D1]) (Exponent 0))

-- | Decodes a 'Double' to a pure Haskell type which holds identical
-- information.
toBCD :: Double -> IO Decoded
toBCD d =
  withForeignPtr (unDouble d) $ \pD ->
  allocaBytes c'DECDOUBLE_Pmax $ \pArr ->
  alloca $ \pExp ->
  c'decDoubleToBCD pD pExp pArr >>= \sgn ->
  peek pExp >>= \ex ->
  peekArray c'DECDOUBLE_Pmax pArr >>= \coef ->
...
--
-- Like decDoubleToString, the payload of an NaN is not shown if it is
-- zero.
...
-- to a 'Double' using 'fromByteString', the number of significant
...
-- # decDouble functions not recreated here:
