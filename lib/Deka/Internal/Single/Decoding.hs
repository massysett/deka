{-# LANGUAGE Safe #-}
module Deka.Internal.Single.Decoding where
...
instance Enum Coefficient where
  toEnum i
    | i < 0 = error $ "Deka.Single.Coefficient.toEnum: argument "
      ++ "out of range; is negative"
    | length r > coefficientLen = error $ "Deka.Single.Coefficient."
  ...
  fromEnum i
    | r > (fromIntegral (maxBound :: Int)) =
        error $ "Deka.Single.Coefficient.fromEnum:"
...
instance Enum Payload where
  toEnum i
    | i < 0 = error $ "Deka.Single.Payload.toEnum: argument "
      ++ "out of range; is negative"
    | length r > payloadLen = error $ "Deka.Single.Payload."
        ++ "toEnum: argument too large"
    | otherwise = Payload r
    where
      r = integralToDigits i
  ...
  fromEnum i
    | r > (fromIntegral (maxBound :: Int)) =
        error $ "Deka.Single.Payload.fromEnum:"
          ++ " argument too large to fit into Int"
    | otherwise = fromIntegral r
    where
...
coefficientLen :: Int
coefficientLen = c'DECSINGLE_Pmax

-- | Maximum number of digits in a payload.
payloadLen :: Int
payloadLen = c'DECSINGLE_Pmax - 1
...
--
-- Also, the upper bound is different too, becuase decSingle is
...
minMaxExp = (l, h)
  where
    l = c'DECSINGLE_Emin - c'DECSINGLE_Pmax + 1
    h = c'DECSINGLE_Emax - c'DECSINGLE_Pmax + 1
...
minNormalAdj = AdjustedExp c'DECSINGLE_Emin
...
toDecNumberBCD (Decoded s v) = (e, ds, sgn)
  where
    ...
    (e, ds) = case v of
      Infinite -> (c'DECFLOAT_Inf, replicate c'DECSINGLE_Pmax 0)
      NaN n (Payload ps) -> (ns, np)
        where
          ...
          pad = replicate (c'DECSINGLE_Pmax - length ps) 0
      Finite (Coefficient digs) (Exponent ex) ->
        ( fromIntegral ex, pad ++ map digitToInt digs )
        where
          pad = replicate (c'DECSINGLE_Pmax - length digs) 0

-- | Encodes a new 'Single'.
fromBCD :: Decoded -> IO Single
fromBCD dcd =
  newSingle >>= \d ->
  withForeignPtr (unSingle d) $ \pD ->
  let (expn, digs, sgn) = toDecNumberBCD dcd in
  withArray digs $ \pArr ->
  c'decSingleFromBCD pD expn pArr sgn >>
  return d

-- | A Single with coefficient 'D1', exponent 0, and sign 'NonNeg'.
one :: IO Single
one = fromBCD
  $ Decoded NonNeg (Finite (Coefficient [D1]) (Exponent 0))

-- | Decodes a 'Single' to a pure Haskell type which holds identical
-- information.
toBCD :: Single -> IO Decoded
toBCD d =
  withForeignPtr (unSingle d) $ \pD ->
  allocaBytes c'DECSINGLE_Pmax $ \pArr ->
  alloca $ \pExp ->
  c'decSingleToBCD pD pExp pArr >>= \sgn ->
  peek pExp >>= \ex ->
  peekArray c'DECSINGLE_Pmax pArr >>= \coef ->
...
--
-- Like decSingleToString, the payload of an NaN is not shown if it is
-- zero.
...
-- to a 'Single' using 'fromByteString', the number of significant
...
-- # decSingle functions not recreated here:
