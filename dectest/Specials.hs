...
import qualified Deka.Class as S
import Types
import Operand
import Directives
import TestHelpers
...
toSciOrEng fn dirs opS rsS cdS = runTestLog $ do
  ...
  let (rConv, fl) = C.runCtxStatus C.initQuad getOp
      r = fn rConv
...
apply dirs opS rsS cdS = runTestLog $ do
  ...
  getOp <- case opS of
    ...
    _ -> flunk $ "expected 1 operand, got " <>
      (BS8.pack . show . length $ opS)
  let k = getOp >>= D.plus
      (rConv, fl) = C.runCtxStatus C.initQuad k
...
parseClass :: BS8.ByteString -> TestLog S.Class
parseClass bs = case lookup lwr ls of
  ...
  where
    ...
    ls = map (first (map toLower)) S.classes
