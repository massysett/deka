module Deka.Fixed.Double.Tests where
...
import Prelude hiding (round, isInfinite, exponent, isNaN, Double)
...
intConversion n gen fr to = testGroup (n ++ " conversions")
  [ testProperty "convert from C integer to Double and back" $
    forAll round $ \r ->
    forAll gen $ \i ->
    let q = fr i
        (i', fl) = runDoubleStatus $ to r q
    in null fl && i' == i
  ]
...
strTests = testGroup "string conversions"
  [ testProperty ("Decoded -> Double -> ByteString"
      ++ " -> Double -> Decoded") $
    forAll genDecoded $ \d ->
      let q = fromBCD d
          bs = toByteString q
          q' = runDouble $ fromByteString bs
          d' = toBCD q'
  ...
  , testProperty ("fromBCD and (fromByteString . scientific) "
    ...
    forAll genDecoded $ \d ->
    let qD = fromBCD d
        (qS, fl) = runDoubleStatus . fromByteString
  ...
  , testProperty "toByteString -> fromByteString" $
    ...
    let q = fromBCD d
        bs = toByteString q
        (q', fl) = runDoubleStatus . fromByteString $ bs
